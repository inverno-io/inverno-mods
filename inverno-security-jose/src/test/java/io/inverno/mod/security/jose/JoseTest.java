...
import io.inverno.mod.boot.internal.converter.JacksonStringConverter;
import io.inverno.mod.boot.internal.converter.JsonStringMediaTypeConverter;
import io.inverno.mod.boot.internal.converter.TextStringMediaTypeConverter;
...
//		System.out.println(MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(Map.of("keys", List.of(jwk_ec, jwk_oct, jwk_edec, jwk_xec, jwk_rsa, jwk_rsa_pss))));
	...
	public void testJWKJKU() {
		...
		// This won't work since JWK retrieved from JKU are not trusted and will be skipped
		// TODO This can be basically changed in the JWKURLResolver: if the source is trusted then it is safe to assume keys are trusted
		/*String payload = "Burn butcher burn";
		JWS<String> jws_ec = jose.jwsService().builder(String.class)
			.header(header -> header
				.keyId("ec")
