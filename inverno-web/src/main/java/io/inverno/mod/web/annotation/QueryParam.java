 ...
 *
 * <p>
 * A query parameter can be defined as optional when the method parameter is an {@link Optional}, it is otherwise considered as required and a {@link MissingRequiredParameterException} will be thrown
 * if the route is invoked with the parameter missing.
 * </p>
 *
 * <p>
 * If the request contains multiple query parameters with the same name, the first one is bound unless the method parameter is an array, a {@link Collection}, a {@link List} or a {@link Set} in which
 * case all values are bound. The parameter converter may also split raw values around a value separator (eg. value {@code 1,2,3,4} can be bound to a list of integers).
 * </p>
 *
 * <pre>{@code
 * @WebRoute( ... )
