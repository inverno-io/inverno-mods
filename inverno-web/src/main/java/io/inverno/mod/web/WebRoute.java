 ...
 *
 * <p>
 * The path to the resource can be either static or dynamic if a parameterized path is specified as defined by {@link URIBuilder}. When defined with a parameterized path, a router can extract path
 * parameters from the {@link URIMatcher} that matches the request. For instance, path <code>/books/{id}</code> defines path parameter {@code id} and matches paths:
