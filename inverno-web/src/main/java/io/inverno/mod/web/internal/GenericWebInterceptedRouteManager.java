 ...
 * @since 1.3
 */
class GenericWebInterceptedRouteManager extends AbstractWebManager<GenericWebInterceptedRouteManager> implements WebRouteManager<ExchangeContext, WebInterceptedRouter<ExchangeContext>> {

	private final GenericWebInterceptedRouter router;

	private WebExchangeHandler<ExchangeContext> handler;
	...
	@Override
	public GenericWebInterceptedRouter disable() {
		this.findRoutes().stream().forEach(route -> route.disable());
		return this.router;
	}

	@Override
	public GenericWebInterceptedRouter remove() {
