...
public class Http1xConnectionV2 extends ChannelDuplexHandler implements HttpConnection {
	...
	private Http1xExchangeV2<?> requestedExchange;
	private Http1xExchangeV2<?> requestingExchange;
	private Http1xExchangeV2<?> respondingExchange;
...
//	@Override
	...
	private void dispose(Throwable throwable) {
		...
		while(current != null) {
			Http1xExchangeV2<?> next = current.next;
			current.next = null;
		...
		if(this.channelContext.executor().inEventLoop()) {
			if(this.requestingExchange.request().isHeadersWritten()) {
				...
				if(this.respondingExchange == this.requestingExchange) {
					...
					Http1xExchangeV2<?> current = this.requestingExchange;
					while(current != null) {
						Http1xExchangeV2<?> next = current.next;
						current.next = null;
						current.dispose(throwable);
