...
class Http1xRequestV2 implements HttpConnectionRequest {
	...
	@Override
	public Http1xRequestBodyV2 body() {
		return this.body;
	}
	...
	private class MonoBodyDataSubscriber extends BaseSubscriber<ByteBuf> {
		...
		protected void hookOnSubscribe(Subscription subscription) {
			Http1xRequestV2.this.disposable = this;
			subscription.request(1);
		}
		
		@Override
		protected void hookOnNext(ByteBuf value) {
			Http1xRequestV2.this.transferedLength += value.readableBytes();
			this.data = value;
		}

		@Override
		protected void hookOnComplete() {
			if(!Http1xRequestV2.this.headers.contains((CharSequence)Headers.NAME_CONTENT_LENGTH)) {
				List<String> transferEncodings = Http1xRequestV2.this.headers.getAll((CharSequence)Headers.NAME_TRANSFER_ENCODING);
				if(transferEncodings.isEmpty() || !transferEncodings.getLast().endsWith(Headers.VALUE_CHUNKED)) {
					// set content length
					Http1xRequestV2.this.headers.add((CharSequence)Headers.NAME_CONTENT_LENGTH, "" + Http1xRequestV2.this.transferedLength);
				}
			}
			if(this.data != null) {
				Http1xRequestV2.this.connection.writeHttpObject(new FlatFullHttpRequest(Http1xRequestV2.this.connection.getVersion(), HttpMethod.valueOf(Http1xRequestV2.this.method.name()), Http1xRequestV2.this.path, Http1xRequestV2.this.headers.unwrap(), this.data, EmptyHttpHeaders.INSTANCE));
			}
			else {
				Http1xRequestV2.this.connection.writeHttpObject(new FlatFullHttpRequest(Http1xRequestV2.this.connection.getVersion(), HttpMethod.valueOf(Http1xRequestV2.this.method.name()), Http1xRequestV2.this.path, Http1xRequestV2.this.headers.unwrap(), Unpooled.EMPTY_BUFFER, EmptyHttpHeaders.INSTANCE));
			}
			Http1xRequestV2.this.headers.setWritten(true);
			Http1xRequestV2.this.connection.onRequestSent();
		}
		
		@Override
		protected void hookOnError(Throwable throwable) {
			Http1xRequestV2.this.connection.onRequestError(throwable);
		}
	}
	...
	private class BodyDataSubscriber extends BaseSubscriber<ByteBuf> {
		...
		private void sanitizeResponse() {
			if(!Http1xRequestV2.this.headers.contains((CharSequence)Headers.NAME_CONTENT_LENGTH)) {
				...
				if(!this.many && (transferEncodings.isEmpty() || !transferEncodings.getLast().endsWith(Headers.VALUE_CHUNKED))) {
					...
					Http1xRequestV2.this.headers.add((CharSequence)Headers.NAME_TRANSFER_ENCODING, (CharSequence)Headers.VALUE_CHUNKED);
				}
			}
		}
		...
		protected void hookOnNext(ByteBuf value) {
			...
			if(!this.many && this.singleChunk == null) {
				...
				this.many = true;
				if(!Http1xRequestV2.this.headers.isWritten()) {
					this.sanitizeResponse();
					Http1xRequestV2.this.connection.writeHttpObject(new FlatHttpRequest(Http1xRequestV2.this.connection.getVersion(), HttpMethod.valueOf(Http1xRequestV2.this.method.name()), Http1xRequestV2.this.path, Http1xRequestV2.this.headers.unwrap(), this.singleChunk));
					Http1xRequestV2.this.headers.setWritten(true);
		...
		protected void hookOnComplete() {
			if(this.many) {
				Http1xRequestV2.this.connection.writeHttpObject(LastHttpContent.EMPTY_LAST_CONTENT);
			}
			else {
				this.sanitizeResponse();
				if(this.singleChunk == null) {
					...
					Http1xRequestV2.this.connection.writeHttpObject(new FlatFullHttpRequest(Http1xRequestV2.this.connection.getVersion(), HttpMethod.valueOf(Http1xRequestV2.this.method.name()), Http1xRequestV2.this.path, Http1xRequestV2.this.headers.unwrap(), this.singleChunk, EmptyHttpHeaders.INSTANCE));
					this.singleChunk = null;
				}
				Http1xRequestV2.this.headers.setWritten(true);
			}
	...
	private class FileRegionBodyDataSubscriber extends BaseSubscriber<FileRegion> {
		...
		@Override
		protected void hookOnSubscribe(Subscription subscription) {
			Http1xRequestV2.this.connection.writeHttpObject(new FlatHttpRequest(Http1xRequestV2.this.connection.getVersion(), HttpMethod.valueOf(Http1xRequestV2.this.method.name()), Http1xRequestV2.this.path, Http1xRequestV2.this.headers.unwrap()));
			Http1xRequestV2.this.headers.setWritten(true);
			subscription.request(1);
		}
		...
		protected void hookOnNext(FileRegion value) {
			Http1xRequestV2.this.transferedLength += value.count();
			Http1xRequestV2.this.connection.writeFileRegion(value, Http1xRequestV2.this.connection.newPromise().addListener(future -> {
				if(future.isSuccess()) {
					this.request(1);
				}
				else {
