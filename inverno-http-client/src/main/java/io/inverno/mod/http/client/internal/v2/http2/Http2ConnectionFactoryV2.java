...
public class Http2ConnectionFactoryV2 implements HttpConnectionFactory<Http2ConnectionV2> {
	...
	public Http2ConnectionV2 create(HttpClientConfiguration configuration, HttpVersion httpVersion, EndpointChannelConfigurer configurer) {
		return new Http2ConnectionFactoryV2.Http2ChannelHandlerBuilder(configuration).build();
	}
	 ...
	 * @since 1.6
	 */
	private class Http2ChannelHandlerBuilder extends AbstractHttp2ConnectionHandlerBuilder<Http2ConnectionV2, Http2ChannelHandlerBuilder> {
		...
		@Override
		protected Http2ConnectionV2 build(Http2ConnectionDecoder decoder, Http2ConnectionEncoder encoder, Http2Settings initialSettings) throws Exception {
			...
			if(this.configuration.compression_enabled()) {
				encoder = new CompressorHttp2ConnectionEncoder(
					encoder,
					Http2ConnectionFactoryV2.this.compressionOptionsProvider.get()
				);
			}
			
			Http2ConnectionV2 connection = new Http2ConnectionV2(
				decoder, 
				encoder, 
				initialSettings, 
				this.configuration, 
				Http2ConnectionFactoryV2.this.headerService,
				Http2ConnectionFactoryV2.this.parameterConverter
			);
		...
		@Override
		public Http2ConnectionV2 build() {
			return super.build();
		}
	}
}
