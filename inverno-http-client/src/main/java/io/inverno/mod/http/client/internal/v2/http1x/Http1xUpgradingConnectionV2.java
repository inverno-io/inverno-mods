...
public class Http1xUpgradingConnectionV2 extends Http1xConnectionV2 {
	...
	private Http1xUpgradingExchangeV2 upgradingExchange;
	...
	private void acceptUpgrade() {
		...
		this.configurer.completeHttp2UpgradeV2(this.channelContext.pipeline(), configuration, this.upgradingExchange, this.messageBuffer);
	...
	protected <A extends ExchangeContext> Http1xExchangeV2 createExchange(Sinks.One<HttpConnectionExchange<A, ? extends HttpConnectionRequest, ? extends HttpConnectionResponse>> sink, EndpointExchange<A> endpointExchange) {
		if(this.state == UpgradeState.COMPLETED) {
			...
			Http1xUpgradingExchangeV2<A> exchange = new Http1xUpgradingExchangeV2<>(this.configuration, sink, this.headerService, this.parameterConverter, endpointExchange.context(), this, endpointExchange.request());
			this.configurer.startHttp2UpgradeV2(this.channelContext.pipeline(), this.configuration, exchange);
