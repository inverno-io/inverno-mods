 ...
 *
 */
public class AbstractConfigurationWinterTest extends AbstractWinterTest {

	public AbstractConfigurationWinterTest() {
		super((Function<File, File>)file -> {
			if(new File("../winter-configuration").exists()) {
				if(file.getName().startsWith("winter-configuration-compiler")) {
					return Optional.of(new File("../winter-configuration-compiler/target/classes")).filter(File::exists).orElse(file);
				}
				else if(file.getName().startsWith("winter-configuration")) {
