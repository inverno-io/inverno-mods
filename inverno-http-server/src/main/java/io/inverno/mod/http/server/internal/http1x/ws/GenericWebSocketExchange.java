...
public class GenericWebSocketExchange extends BaseSubscriber<WebSocketFrame> implements WebSocketExchange<ExchangeContext> {
	...
	/**
	 * <p>
	 * Finalizes the exchange by invoking completing the inbound sink (if present) and by subscribing to the finalizer (if present). 
	 * </p>
	 * 
			...
			return this.frames
				...
				.windowUntil(frame -> {
					if(this.currentFrameKind == null) {
						...
						if(this.currentFrameKind == WebSocketFrame.Kind.CONTINUATION) {
							// We can't receive a continuation frame before a non-final TEXT or BINARY frame
				...
				.map(messageFrames -> {
					...
					if(null == this.currentFrameKind) {
						...
						case TEXT: {
							message = new GenericWebSocketMessage(WebSocketMessage.Kind.TEXT, messageFrames);
							break;
						}
						case BINARY: {
							message = new GenericWebSocketMessage(WebSocketMessage.Kind.BINARY, messageFrames);
							break;
						}
						default: {
							// Should never happen
