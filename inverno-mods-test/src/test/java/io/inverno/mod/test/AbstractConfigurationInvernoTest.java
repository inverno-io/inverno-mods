 ...
 *
 */
public class AbstractConfigurationInvernoTest extends AbstractInvernoTest {

	public AbstractConfigurationInvernoTest() {
		super((Function<File, File>)file -> {
			if(new File("../inverno-configuration").exists()) {
				if(file.getName().startsWith("inverno-configuration-compiler")) {
					...
					return Optional.of(new File("../../inverno/inverno-core-annotation/target/classes")).filter(File::exists).orElse(file);
				}
				else if(file.getName().startsWith("inverno-core-compiler")) {
					return Optional.of(new File("../../inverno/inverno-core-compiler/target/classes")).filter(File::exists).orElse(file);
				}
				else if(file.getName().startsWith("inverno-core")) {
					return Optional.of(new File("../../inverno/inverno-core/target/classes")).filter(File::exists).orElse(file);
				}
			}
			return file;
		},
