...
public class UrlEncodedBodyDecoder implements RequestBodyDecoder<Parameter> {

	public static void main(String[] args) {
		
		
		/*QueryStringDecoder dec = new QueryStringDecoder("http://toto?tata=5&toto=6&titi&&&tutu");
		dec.parameters().entrySet().stream().forEach(e -> {
		...
		ByteBuf buffer3 = Unpooled.unreleasableBuffer(Unpooled.copiedBuffer("sqfdsfd=45", CharsetUtil.UTF_8));
		
		Sinks.Many<ByteBuf> sink = Sinks.many().multicast().onBackpressureBuffer();
		
		decoder.decode(sink.asFlux(), contentType).subscribe(
			parameter -> {
		...
		sink.tryEmitNext(buffer1);
		sink.tryEmitNext(buffer2);
//		sink.tryEmitNext(buffer3);
