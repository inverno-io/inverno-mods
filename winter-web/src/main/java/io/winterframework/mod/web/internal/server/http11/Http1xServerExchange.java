		...
		try {
			...
			if(!headers.isWritten()) {
				...
				if(headers.getSize() == null && !transferEncodings.contains("chunked")) {
					...
					this.manageChunked = !headers.getContentType().getMediaType().equals(MediaTypes.TEXT_EVENT_STREAM);
				}
				
				/*HttpResponse response = new DefaultHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.OK);
				headers.getAll().values().stream()
					.flatMap(List::stream)
					.forEach(header -> {
						if(header.getHeaderName().equals(Headers.PSEUDO_STATUS)) {
							response.setStatus(HttpResponseStatus.valueOf(Integer.parseInt(header.getHeaderValue())));
						}
						else {
							response.headers().add(header.getHeaderName(), header.getHeaderValue());
						}
					});*/
