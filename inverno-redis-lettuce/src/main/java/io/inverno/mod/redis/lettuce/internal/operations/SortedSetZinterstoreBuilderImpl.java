 ...
 * @param <C>
 */
public class SortedSetZinterstoreBuilderImpl<A, B, C extends StatefulConnection<A, B>> implements RedisSortedSetReactiveOperations.SortedSetZinterstoreBuilder<A, B> {
	 ...
	 * @param keyType 
	 */
	public SortedSetZinterstoreBuilderImpl(RedisSortedSetReactiveCommands<A, B> commands, Class<A> keyType) {
	 ...
	 * @param keyType 
	 */
	public SortedSetZinterstoreBuilderImpl(Mono<StatefulRedisConnectionOperations<A, B, C, ?>> connection, Class<A> keyType) {
		this.commands = null;
	...
	@Override
	public SortedSetZinterstoreBuilderImpl<A, B, C> weight(double weight) {
	...
	@Override
	public SortedSetZinterstoreBuilderImpl<A, B, C> sum() {
		this.sum = true;
		this.min = false;
		this.max = false;
		return this;
	...
	@Override
	public SortedSetZinterstoreBuilderImpl<A, B, C> min() {
		this.sum = false;
		this.min = true;
		this.max = false;
		return this;
	...
	@Override
	public SortedSetZinterstoreBuilderImpl<A, B, C> max() {
		this.sum = false;
		this.min = false;
		this.max = true;
		return this;
	...
	private Mono<Long> build(RedisSortedSetReactiveCommands<A, B> localCommands, A destination, A key) {
		return localCommands.zinterstore(destination, this.buildZStoreArgs(), key);
	}
	...
	private Mono<Long> build(RedisSortedSetReactiveCommands<A, B> localCommands, A destination, Consumer<Keys<A>> keys) {
		...
		return localCommands.zinterstore(destination, this.buildZStoreArgs(), keysConfigurator.getKeys());
	}
}
