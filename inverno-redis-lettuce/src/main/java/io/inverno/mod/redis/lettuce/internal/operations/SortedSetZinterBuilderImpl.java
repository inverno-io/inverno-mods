 ...
 * @param <C>
 */
public class SortedSetZinterBuilderImpl<A, B, C extends StatefulConnection<A, B>> implements RedisSortedSetReactiveOperations.SortedSetZinterBuilder<A, B> {
	 ...
	 * @param keyType 
	 */
	public SortedSetZinterBuilderImpl(RedisSortedSetReactiveCommands<A, B> commands, Class<A> keyType) {
	 ...
	 * @param keyType 
	 */
	public SortedSetZinterBuilderImpl(Mono<StatefulRedisConnectionOperations<A, B, C, ?>> connection, Class<A> keyType) {
		this.commands = null;
	...
	@Override
	public SortedSetZinterBuilderImpl<A, B, C> weight(double weight) {
	...
	@Override
	public SortedSetZinterBuilderImpl<A, B, C> sum() {
		this.sum = true;
		this.min = false;
		this.max = false;
		return this;
	...
	@Override
	public SortedSetZinterBuilderImpl<A, B, C> min() {
		this.sum = false;
		this.min = true;
		this.max = false;
		return this;
	...
	@Override
	public SortedSetZinterBuilderImpl<A, B, C> max() {
		this.sum = false;
		this.min = false;
		this.max = true;
		return this;
	...
	private Flux<B> build(RedisSortedSetReactiveCommands<A, B> localCommands, A key) {
		return localCommands.zinter(this.buildZAggregateArgs(), key);
	}
