 ...
 * @param <C>
 */
public class KeyExpireatBuilderImpl<A, B, C extends StatefulConnection<A, B>> implements RedisKeyReactiveOperations.KeyExpireatBuilder<A> {
	 ...
	 * @param commands 
	 */
	public KeyExpireatBuilderImpl(RedisKeyReactiveCommands<A, B> commands) {
	 ...
	 * @param connection 
	 */
	public KeyExpireatBuilderImpl(Mono<StatefulRedisConnectionOperations<A, B, C, ?>> connection) {
		this.commands = null;
		this.connection = connection;
	}

	@Override
	public KeyExpireatBuilderImpl<A, B, C> nx() {
		this.nx = true;
		this.xx = false;
		this.gt = false;
		this.lt = false;
	...
	@Override
	public KeyExpireatBuilderImpl<A, B, C> xx() {
		this.nx = false;
		this.xx = true;
		this.gt = false;
		this.lt = false;
	...
	@Override
	public KeyExpireatBuilderImpl<A, B, C> gt() {
		this.nx = false;
		this.xx = false;
		this.gt = true;
		this.lt = false;
	...
	@Override
	public KeyExpireatBuilderImpl<A, B, C> lt() {
		this.nx = false;
		this.xx = false;
		this.gt = false;
		this.lt = true;
	...
	private Mono<Boolean> build(RedisKeyReactiveCommands<A, B> localCommands, A key, long epochSeconds) {
		...
		return localCommands.expireat(key, epochSeconds);
	}
	...
	private Mono<Boolean> build(RedisKeyReactiveCommands<A, B> localCommands, A key, ZonedDateTime datetime) {
		// this will throw an UnsupportedOperationException when one of nx, xx, gt and lt is specified as it is not supported yet
		this.buildExpireatArgs();
		return localCommands.expireat(key, datetime.toInstant());
	}
	...
	private Mono<Boolean> build(RedisKeyReactiveCommands<A, B> localCommands, A key, Instant instant) {
		// this will throw an UnsupportedOperationException when one of nx, xx, gt and lt is specified as it is not supported yet
		this.buildExpireatArgs();
		return localCommands.expireat(key, instant);
	}
}
